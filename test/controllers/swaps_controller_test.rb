...
class SwapsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create swap" do
    assert_difference('Swap.count') do
      post swaps_url, params: { swap: { comment: @swap.comment, game_id: @swap.game_id, status: @swap.status, ticket_id: @swap.ticket_id } }, as: :json
    end
  ...
  test "should update swap" do
    patch swap_url(@swap), params: { swap: { comment: @swap.comment, game_id: @swap.game_id, status: @swap.status, ticket_id: @swap.ticket_id } }, as: :json
    assert_response 200
  end
